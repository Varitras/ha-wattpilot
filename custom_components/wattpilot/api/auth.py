...
from .models import AuthHashType

# bcrypt.js custom base64 alphabet
_BASE64_CODE = list("./ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789")


# bcrypt writes its cost as two digits, so a single-digit one is padded.
_BCRYPT_COST_TWO_DIGITS = 10
...
def _hash_bcrypt(password: str, serial: str, iterations: int = 8) -> bytes:
    ...
    if iterations < _BCRYPT_COST_TWO_DIGITS:
        salt_parts.append("0")
...
def _bcryptjs_base64_encode(b: bytes, length: int) -> str:
    """Port of bcrypt.js ``encodeBase64``, as the charger firmware expects it."""
...
def sign_secured_message(
    message: dict[str, Any], hashed_password: bytes
) -> dict[str, Any]:
